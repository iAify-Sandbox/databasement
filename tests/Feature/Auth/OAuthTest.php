...
test('oauth callback creates new user when email not found', function () {
    ...
    expect($user)->not->toBeNull()
        ...
        ->and($user->roleNameIn(\App\Models\Organization::default()))->toBe('member')
...
test('oauth uses configured default role for new users', function () {
    ...
    expect($user->roleNameIn(\App\Models\Organization::default()))->toBe('viewer');
});
...
test('oidc role mapping assigns admin role from matching group', function () {
    ...
    expect($user->roleNameIn(\App\Models\Organization::default()))->toBe('admin');
});
...
test('oidc role mapping assigns member role from matching group', function () {
    ...
    expect($user->roleNameIn(\App\Models\Organization::default()))->toBe('member');
});
...
test('oidc role mapping assigns operator role from matching group', function () {
    ...
    expect($user->roleNameIn(\App\Models\Organization::default()))->toBe('operator');
});
...
test('new oauth user joins configured default organization', function () {
    ...
    expect($user)->not->toBeNull()
        ...
        ->and($user->roleNameIn($customOrg))->toBe('member');
});
...
test('oidc role mapping syncs role in configured default organization', function () {
    ...
    $user = User::factory()->create(['role' => 'member']);
    attachUserToOrg($user, $customOrg, 'member');
    OAuthIdentity::create([
        'user_id' => $user->id,
        'provider' => 'oidc',
    ...
    $user->refresh();
    expect($user->roleNameIn($customOrg))->toBe('admin');
});
